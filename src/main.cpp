...
std::vector<neutron_struct> neutrons;

// The reactor core itself, each are channels in the x and y dimensions
std::vector<std::vector<channel_struct>> reactor_core;
...
static void updateNeutronsPosition(int, struct simulation_configuration_struct*);
static void handleNeutronsActivity(int, struct simulation_configuration_struct*);
static void updateFuelAssembly(int, struct channel_struct*, struct simulation_configuration_struct*);
static void updateNeutronGenerator(int, struct channel_struct*, struct simulation_configuration_struct*);
static void createNeutrons(int, struct channel_struct*, double, struct simulation_configuration_struct*);
...
static void reorganize_active_neutrons();

static int rank = 0;
static long int total_neutrons, local_neutrons = 0;
...
{
	bool successful = initialize_mpi(argc, argv);
	...
	if(rank == ROOT && argc != 3)
	{
		printf("You must provide two arguments, the configuration filename and output filename to "
			   "the code\n");
		finalize_mpi();
		return -1;
	}
	...
	// Initialize neutrons
	neutrons.resize(configuration.max_neutrons);
	...
	if(rank == ROOT)
	{
		...
		printf("Simulation configured for reactor core of size %dm by %dm by %dm, timesteps=%d dt=%dns\n",
			   ...
			   configuration.size_y,
			   configuration.size_z,
	...
	if(rank == ROOT)
	{
		...
		printf("Model completed after %d timesteps\nTotal model time: %f secs\nTotal fissions: %ld "
			   "releasing %e MeV and %e Joules\nTotal runtime: %.2f seconds\n",
			   configuration.num_timesteps,
			   (NS_AS_SEC * configuration.dt) * configuration.num_timesteps,
			   num_fissions,
			   mev,
			   joules,
...
{
	local_neutrons = distribute_neutrons_from_root(neutrons, local_neutrons);
	updateNeutronsPosition(dt, configuration);
	handleNeutronsActivity(dt, configuration);
	reduce_int_sum_in_root(local_neutrons, &total_neutrons);
	if(rank == ROOT)
	{
 ...
 **/
static void
generateReport(int dt, int timestep, struct simulation_configuration_struct* configuration, struct timeval start_time)
{
	unsigned long int num_fissions = getTotalNumberFissions(configuration);
	double mev = getMeVFromFissions(num_fissions);
	double joules = getJoulesFromMeV(mev);
	printf("Timestep: %d, model time is %e secs, current runtime is %.2f seconds. %ld active "
		   "neutrons, %ld fissions, releasing %e MeV and %e Joules\n",
		   timestep,
		   (NS_AS_SEC * dt) * timestep,
		   getElapsedTime(start_time),
		   total_neutrons,
		   num_fissions,
		   mev,
...
static void updateReactorCore(int dt, struct simulation_configuration_struct* configuration)
{
	for(int i = 0; i < configuration->channels_x; ++i)
	{
		for(int j = 0; j < configuration->channels_y; ++j)
		{
			...
			{
				updateFuelAssembly(dt, &(reactor_core[i][j]), configuration);
			}
...
/**
 * Reorganize the active and inactive neutrons in the `neutrons` array.
 */
static void reorganize_active_neutrons()
{
	...
	// Iterate through the local neutron array and swap inactive neutrons to the end of the array.
	while(i < local_neutrons)
	{
		// If neutron is inactive, swap it with the last active neutron in the array and decrement
		// local_neutrons.
		if(!neutrons[i].active)
		{
			std::swap(neutrons[i], neutrons[local_neutrons - 1]);
			local_neutrons = local_neutrons - 1;
			continue;
		}
 ...
 **/
static void updateNeutronsPosition(int dt, struct simulation_configuration_struct* configuration)
{
	std::vector<neutron_struct> neutrons_in_fuel_channel;

#pragma omp parallel for default(none) shared(dt, configuration, local_neutrons, neutrons, neutrons_in_fuel_channel)
	for(size_t i = 0; i < local_neutrons; ++i)
	{
		// Rest mass is 1 for a neutron
		double total_velocity = MeVToVelocity(neutrons[i].energy, 1);
		// These components are positive or negative which denote movement in one direction or
		// another
		...
		struct channel_struct* reactorChannel =
			locateChannelFromPosition(neutrons[i].pos_x, neutrons[i].pos_y, configuration);
		if(reactorChannel != NULL && reactorChannel->type == FUEL_ASSEMBLY)
		{
#pragma omp critical
			{
				neutrons_in_fuel_channel.push_back(neutrons[i]);
			}
			neutrons[i].active = false;
		}
	}
	reorganize_active_neutrons();
	local_neutrons = append_neutrons_to_root(neutrons, neutrons_in_fuel_channel, local_neutrons);
}

/**
 * Handle the collision of the neutron with a fuel channel, moderator, or control rod
 */
static void handleNeutronsActivity(int dt, struct simulation_configuration_struct* configuration)
{

#pragma omp parallel for default(none) shared(dt, configuration, local_neutrons, neutrons, stderr)
	for(size_t i = 0; i < local_neutrons; ++i)
	{
		// Now figure out if neutron is in a fuel assembly, moderator or control rod. If so then
		...
		if(reactorChannel != NULL)
		{
			...
			{
				// It is in a fuel assembly channel, determine if it has collided with a neutron
...
#pragma omp critical
					{
						collision = determineAndHandleIfNeutronFuelCollision(
							neutrons[i].energy, reactorChannel, fuel_pellet, configuration->collision_prob_multiplyer);
					}
			...
			{
				// The neutron is in the moderator, determine if it has been absorbed by the
				// moderator or ot
				bool absorbed = determineAndHandleIfNeutronModeratorCollision(&neutrons[i],
																			  configuration->moderator_weight,
																			  reactorChannel->contents.moderator.type,
																			  configuration->size_z);
				if(absorbed)
				{
					neutrons[i].active = false;
			...
			if(reactorChannel->type == CONTROL_ROD)
			{
				...
				{
					// Has hit the control rod, therefore this absorbed and removed from
					// simulation
					neutrons[i].active = false;
				}
			}
		...
		{
			fprintf(stderr,
					"Unable to locate reactor core channel for x=%f and y=%f\n",
					neutrons[i].pos_x,
					neutrons[i].pos_y);
			finalize_mpi();
			exit(-1);
...
/**
 * Update the state of a specific fuel assembly in a channel for a timestep. This will fission all
 * U236 and Pu239 in the assembly and update the constituent components as required
 **/
static void
updateFuelAssembly(int dt, struct channel_struct* channel, struct simulation_configuration_struct* configuration)
{
	for(int i = 0; i < channel->contents.fuel_assembly.num_pellets; ++i)
	{
		...
		for(unsigned long int j = 0; j < num_u236; ++j)
		{
			...
			createNeutrons(
				num_neutrons, channel, (i * HEIGHT_FUEL_PELLET_M) + (HEIGHT_FUEL_PELLET_M / 2), configuration);
 ...
 **/
static void
updateNeutronGenerator(int dt, struct channel_struct* channel, struct simulation_configuration_struct* configuration)
{
	unsigned long int number_new_neutrons =
		getNumberNeutronsFromGenerator(channel->contents.neutron_generator.weight, dt);
	for(int i = 0; i < number_new_neutrons; ++i)
	{
		if(total_neutrons == configuration->max_neutrons)
			break;

		initialiseNeutron(&(neutrons[local_neutrons]),
						  channel->x_centre,
						  channel->y_centre,
						  (double)(rand() / ((double)(RAND_MAX / configuration->size_z))));
		total_neutrons = total_neutrons + 1;
		local_neutrons = local_neutrons + 1;
	}
}
 ...
 **/
static void createNeutrons(int num_neutrons,
						   ...
						   struct simulation_configuration_struct* configuration)
{
	for(int k = 0; k < num_neutrons; ++k)
	{
		if(total_neutrons == configuration->max_neutrons)
			break;
		initialiseNeutron(&(neutrons[local_neutrons]), channel->x_centre, channel->y_centre, z);
		total_neutrons = total_neutrons + 1;
		local_neutrons = local_neutrons + 1;
	}
}
...
{
	reactor_core.resize(simulation_configuration->channels_x);
	for(int i = 0; i < simulation_configuration->channels_x; i++)
	{
		reactor_core[i].resize(simulation_configuration->channels_y);
	}
	for(int i = 0; i < simulation_configuration->channels_x; ++i)
	{
		...
		else
		{
			...
			{
				// For every configuration that was provided read what that was and initialise
				// channel as required
				reactor_core[i][j].x_centre = centre_x;
				reactor_core[i][j].y_centre = ((j * CHANNEL_SIZE) + (CHANNEL_SIZE / 2));
				...
				else if(simulation_configuration->channel_layout_config[i][j] == CONFIG_FUEL_ASSEMBLY)
				{
					...
					reactor_core[i][j].contents.fuel_assembly.num_pellets =
						simulation_configuration->size_z / FUEL_PELLET_DEPTH;
					reactor_core[i][j].contents.fuel_assembly.num_fissions = 0;
					reactor_core[i][j].contents.fuel_assembly.quantities =
						(double(*)[NUM_CHEMICALS])malloc(sizeof(unsigned long int[NUM_CHEMICALS]) *
														 reactor_core[i][j].contents.fuel_assembly.num_pellets);
					for(int z = 0; z < reactor_core[i][j].contents.fuel_assembly.num_pellets; ++z)
					{
						// For each pellet in the assembly set up the number of atoms present for
						// each chemical, these will change as the simulation progresses and
						// (hopefully!) fission occurs
						for(int k = 0; k < NUM_CHEMICALS; ++k)
						{
							...
							reactor_core[i][j].contents.fuel_assembly.quantities[z][k] =
								getAtomsPerGram(chemical) *
								(simulation_configuration->fuel_makeup_percentage[k] / 100.0);
						}
					}
				}
				else if(simulation_configuration->channel_layout_config[i][j] == CONFIG_CONTROL_ROD)
				{
					// If the channel is a control rod then set this and store the absolute z
					// location it is lowered to
					reactor_core[i][j].type = CONTROL_ROD;
					reactor_core[i][j].contents.control_rod.lowered_to_level =
						getControlRodLoweredToLevel(simulation_configuration, i, j);
				}
				...
				else if(simulation_configuration->channel_layout_config[i][j] == CONFIG_NEUTRON_GENERATOR)
				{
					...
					reactor_core[i][j].contents.neutron_generator.weight =
						simulation_configuration->size_z * 100 * NEUTRON_GENERATOR_WEIGHT_PER_CM;
				}
			}
			for(int j = simulation_configuration->num_channel_configs[i]; j < simulation_configuration->channels_y; ++j)
			{
				// For any remaining channels that were not explicitly configured for this row, then
				// set them as empty
				reactor_core[i][j].type = EMPTY_CHANNEL;
				reactor_core[i][j].x_centre = centre_x;
				reactor_core[i][j].y_centre = ((j * CHANNEL_SIZE) + (CHANNEL_SIZE / 2));
...
/**
 * For a control rod channel will return the absolute z height position that this has been lowered
 * to based upon the percentage insertion that was configured
 **/
static double getControlRodLoweredToLevel(struct simulation_configuration_struct* simulation_configuration,
										  int channel_x,
										  int channel_y)
{
	...
	{
		fprintf(
			stderr, "Expected control rod configuration for x=%d y=%d but none can be found\n", channel_x, channel_y);
		exit(-1);
	}
	return simulation_configuration->size_z *
		   (simulation_configuration->control_rod_configurations[rodConfigurationIndex].percentage / 100.0);
}
...
static void writeReactorState(struct simulation_configuration_struct* configuration, int timestep, char* outfile)
{
	...
	fprintf(f,
			"Reactor state at time %e secs, %ld fissions releasing %e MeV and %e Joules\n",
			(NS_AS_SEC * configuration->dt) * timestep,
			num_fissions,
			mev,
			joules);
	fprintf(f, "----------------------------------------------------------------------------\n");
	for(int i = 0; i < configuration->channels_x; ++i)
	{
		for(int j = 0; j < configuration->channels_y; ++j)
		{
			if(reactor_core[i][j].type == FUEL_ASSEMBLY)
			{
				...
				fprintf(f,
						"Fuel assembly %d %d, %e U235 %e U238 %e Pu239 %e U236 %e Ba141 %e Kr92 %e "
						"Xe140 %e Sr94 %e Xe134 %e Zr103 %e Pu240\n",
						i,
						j,
						pc[0],
						pc[1],
...
/**
 * Clears out the file that we are going to write to for the reactor state, this is called at
 * simulation startup and it will overwrite any existing contents
 **/
...
locateChannelFromPosition(double x, double y, struct simulation_configuration_struct* configuration)
{
...
/**
 * Based upon the properties of each fuel assembly will return the total number of fissions that
 * have occured across all fuel assemblies in the simulation.
 **/
